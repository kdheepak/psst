...
def initialize_generators(model,
    ...
    model.Generators = Set(initialize=model.ThermalGenerators | model.RenewableGenerators | model.HydroGenerators)
    model.GeneratorsAtBus = Set(model.Buses, initialize=generator_at_bus)


def initialize_thermal_generators(model,
                        thermal_generator_names=None):

    model.ThermalGenerators = Set(initialize=thermal_generator_names)


def initialize_renewable_generators(model,
                                        renewable_generator_names=None,renewable_gen=None):

    model.RenewableGenerators = Set(initialize=renewable_generator_names)
    model.renewable_power = Param(model.RenewableGenerators, model.TimePeriods, initialize=renewable_gen, default=0.0, mutable=True)
    
def initialize_hydro_generators(model,
                                        hydro_generator_names=None,hydro_gen=None):

    model.HydroGenerators = Set(initialize=hydro_generator_names)    
    model.hydro_power = Param(model.HydroGenerators, model.TimePeriods, initialize=hydro_gen, default=0.0, mutable=True)
